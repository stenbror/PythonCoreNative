...
std::shared_ptr<AST::ExpressionNode> PythonCoreParser::ParseAtom()
{
    ...
    switch (curSymbol->GetSymbolKind())
    {
        ...
        case TokenKind::PyLeftParen:
        
            {
                ...
                if (mLexer->CurSymbol()->GetSymbolKind() == TokenKind::PyYield)
                {
                    ...
                    if (mLexer->CurSymbol()->GetSymbolKind() != TokenKind::PyRightParen) 
                        throw std::make_shared<SyntaxError>(mLexer->Position(), mLexer->CurSymbol(), std::make_shared<std::wstring>(L"Missing ')' in tuple!"));
                    auto symbol2 = mLexer->CurSymbol();
                    mLexer->Advance();
        ...
        case TokenKind::PyLeftBracket:
        
            {
                ...
                else
                {
                    auto node = ParseTestListComp();
                    if (mLexer->CurSymbol()->GetSymbolKind() != TokenKind::PyRightBracket) 
                        throw std::make_shared<SyntaxError>(mLexer->Position(), mLexer->CurSymbol(), std::make_shared<std::wstring>(L"Missing ']' in list!"));
                    auto symbol2 = mLexer->CurSymbol();
                    mLexer->Advance();
        ...
        case TokenKind::PyLeftCurly:
        
            {
                ...
                else
                {
                    ...
                    if (mLexer->CurSymbol()->GetSymbolKind() != TokenKind::PyRightCurly) 
                        throw std::make_shared<SyntaxError>(mLexer->Position(), mLexer->CurSymbol(), std::make_shared<std::wstring>(L"Missing '}' in dictionary!"));
                    auto symbol2 = mLexer->CurSymbol();
                    mLexer->Advance();
        ...
        default:
            ...
            throw std::make_shared<SyntaxError>(mLexer->Position(), mLexer->CurSymbol(), std::make_shared<std::wstring>(L"Illegal literal found!"));
    }
}
