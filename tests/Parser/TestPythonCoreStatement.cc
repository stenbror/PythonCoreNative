...
TEST_CASE( "Rule: try", "Parser - Statement rules" )
{
    ...
    SECTION( "try: pass\r\nexcept a: pass\r\nfinally: pass\r\n" )
    {
        ...
        REQUIRE( elem1->GetExceptNodes()->size() == 1 );


        // Start of Except

        auto exp1 = std::static_pointer_cast<AST::ExceptNode>( elem1->GetExceptNodes()->at(0) );

        auto exp1_left = std::static_pointer_cast<AST::ExceptClauseNode>( exp1->GetLeft() );
        REQUIRE( exp1_left->GetOperator1()->GetSymbolKind() == TokenKind::PyExcept );
        auto exp1_left_left = std::static_pointer_cast<AST::AtomNameNode>( exp1_left->GetLeft() );
        REQUIRE( exp1_left_left->GetNameText()->GetText()->compare(L"a") == 0 );

        REQUIRE( exp1_left->GetOperator2() == nullptr );
        REQUIRE( exp1_left->GetOperator3() == nullptr );


        REQUIRE( exp1->GetOperator()->GetSymbolKind() == TokenKind::PyColon );

        auto exp1_right = std::static_pointer_cast<AST::SimpleStatementNode>( exp1->GetRight() );
        auto exp1_item_right = std::static_pointer_cast<AST::PassStatementNode>( exp1_right->GetNodes()->at(0) );
        REQUIRE( exp1_item_right->GetOperator()->GetSymbolKind() == TokenKind::PyPass );
        ...
        REQUIRE( elem1->GetNodeStartPosition() == 0 );
        REQUIRE( elem1->GetNodeEndPosition() == 42 );
    ...
    SECTION( "try: pass\r\nexcept a: pass\r\nexcept b as d: pass\r\nfinally: pass\r\n" )
    {
        ...
        REQUIRE( elem1->GetExceptNodes()->size() == 2 );
        ...
        auto exp2 = std::static_pointer_cast<AST::ExceptNode>( elem1->GetExceptNodes()->at(1) );

        auto exp2_left = std::static_pointer_cast<AST::ExceptClauseNode>( exp2->GetLeft() );
        REQUIRE( exp2_left->GetOperator1()->GetSymbolKind() == TokenKind::PyExcept );
        auto exp2_left_left = std::static_pointer_cast<AST::AtomNameNode>( exp2_left->GetLeft() );
        REQUIRE( exp2_left_left->GetNameText()->GetText()->compare(L"b") == 0 );

        REQUIRE( exp2_left->GetOperator2()->GetSymbolKind() == TokenKind::PyAs );
        auto exp2_as_right = std::static_pointer_cast<NameToken>( exp2_left->GetOperator3() );
        REQUIRE( exp2_as_right->GetText()->compare(L"d") == 0 );


        REQUIRE( exp2->GetOperator()->GetSymbolKind() == TokenKind::PyColon );

        auto exp2_right = std::static_pointer_cast<AST::SimpleStatementNode>( exp2->GetRight() );
        auto exp2_item_right = std::static_pointer_cast<AST::PassStatementNode>( exp2_right->GetNodes()->at(0) );
        REQUIRE( exp2_item_right->GetOperator()->GetSymbolKind() == TokenKind::PyPass );
        ...
        REQUIRE( elem1->GetNodeStartPosition() == 0 );
        REQUIRE( elem1->GetNodeEndPosition() == 63 );
    ...
    SECTION( "try: pass\r\nexcept a: pass\r\nexcept b as d: pass\r\nelse: pass\r\nfinally: pass\r\n" )
    {
        ...
        REQUIRE( elem1->GetNodeStartPosition() == 0 );
        REQUIRE( elem1->GetNodeEndPosition() == 75 );
