...
TEST_CASE( "Rule: VarArgsList", "Parser - Expression rules" )
{
    ...
    SECTION( "lambda a, b, c, /, d : x " )
    {
        ...
        auto left = std::static_pointer_cast<AST::VarArgsListExpressionNode>( node->GetLeft() );
        REQUIRE( left->GetNodes()->size() == 4 );
        REQUIRE( left->GetSeparators()->size() == 4 );
        REQUIRE( left->GetSeparators()->at(0)->GetSymbolKind() == TokenKind::PyComma );
        REQUIRE( left->GetSeparators()->at(1)->GetSymbolKind() == TokenKind::PyComma );
        REQUIRE( left->GetSeparators()->at(2)->GetSymbolKind() == TokenKind::PyComma );
        ...
        REQUIRE ( node->GetNodeStartPosition() == 0 ) ;  
        REQUIRE ( node->GetNodeEndPosition() == 25 ) ; 

    }

    SECTION( "lambda a, b, c, /, d, *e : x " )
    {

        auto sourceBuffer = std::make_shared<SourceBuffer>( std::make_shared<std::wstring>( L"lambda a, b, c, /, d, *e : x " ) );
        auto lexer = std::make_shared<PythonCoreTokenizer>(4, sourceBuffer);
        auto parser = std::make_shared<PythonCoreParser>(lexer);
        ...
        auto left = std::static_pointer_cast<AST::VarArgsListExpressionNode>( node->GetLeft() );
        REQUIRE( left->GetNodes()->size() == 4 );
        REQUIRE( left->GetSeparators()->size() == 5 );
        REQUIRE( left->GetSeparators()->at(0)->GetSymbolKind() == TokenKind::PyComma );
        REQUIRE( left->GetSeparators()->at(1)->GetSymbolKind() == TokenKind::PyComma );
        REQUIRE( left->GetSeparators()->at(2)->GetSymbolKind() == TokenKind::PyComma );
        REQUIRE( left->GetSeparators()->at(3)->GetSymbolKind() == TokenKind::PyComma );
    ...
    SECTION( "lambda a, b, c, /, d, *e, f, g : x " )
    {

        auto sourceBuffer = std::make_shared<SourceBuffer>( std::make_shared<std::wstring>( L"lambda a, b, c, /, d, *e, f, g : x " ) );
        auto lexer = std::make_shared<PythonCoreTokenizer>(4, sourceBuffer);
        auto parser = std::make_shared<PythonCoreParser>(lexer);
        ...
        REQUIRE( left->GetSeparators()->size() == 7 );
        REQUIRE( left->GetSeparators()->at(0)->GetSymbolKind() == TokenKind::PyComma );
        REQUIRE( left->GetSeparators()->at(1)->GetSymbolKind() == TokenKind::PyComma );
        REQUIRE( left->GetSeparators()->at(2)->GetSymbolKind() == TokenKind::PyComma );
        REQUIRE( left->GetSeparators()->at(3)->GetSymbolKind() == TokenKind::PyComma );
        ...
        REQUIRE ( node->GetNodeStartPosition() == 0 ) ;  
        REQUIRE ( node->GetNodeEndPosition() == 35 ) ; 
    ...
    SECTION( "lambda a, b, c, /, d, *e, f, g, **h, : x " )
    {
        ...
        auto left = std::static_pointer_cast<AST::VarArgsListExpressionNode>( node->GetLeft() );
        REQUIRE( left->GetNodes()->size() == 6 );
        REQUIRE( left->GetSeparators()->size() == 9 );
        REQUIRE( left->GetSeparators()->at(0)->GetSymbolKind() == TokenKind::PyComma );
        REQUIRE( left->GetSeparators()->at(1)->GetSymbolKind() == TokenKind::PyComma );
        REQUIRE( left->GetSeparators()->at(2)->GetSymbolKind() == TokenKind::PyComma );
        REQUIRE( left->GetSeparators()->at(3)->GetSymbolKind() == TokenKind::PyComma );
        ...
        REQUIRE( left->GetSeparators()->at(6)->GetSymbolKind() == TokenKind::PyComma );
        REQUIRE( left->GetSeparators()->at(7)->GetSymbolKind() == TokenKind::PyComma );
        REQUIRE( left->GetSeparators()->at(8)->GetSymbolKind() == TokenKind::PyComma );
        ...
        REQUIRE ( node->GetNodeStartPosition() == 0 ) ;  
        REQUIRE ( node->GetNodeEndPosition() == 41 ) ; 

    }

    SECTION( "lambda a = 1, b, c, /, d, *e, f, g, **h, : x " )
    {

        auto sourceBuffer = std::make_shared<SourceBuffer>( std::make_shared<std::wstring>( L"lambda a = 1, b, c, /, d, *e, f, g, **h, : x " ) );
        auto lexer = std::make_shared<PythonCoreTokenizer>(4, sourceBuffer);
        auto parser = std::make_shared<PythonCoreParser>(lexer);
        ...
        auto el1 = std::static_pointer_cast<AST::VFPDefAssignExpressionNode>( left->GetNodes()->at(0) );
        auto el1left = std::static_pointer_cast<NameToken>( el1->GetOperator1() );
        REQUIRE( el1left->GetText()->compare(L"a") == 0 );
        REQUIRE( el1->GetOperator2()->GetSymbolKind() == TokenKind::PyAssign );
        auto el1right = std::static_pointer_cast<AST::AtomNumberNode>( el1->getRight() );
        REQUIRE( el1right->GetNumberText()->GetText()->compare(L"1") == 0 );
        ...
        REQUIRE ( node->GetNodeStartPosition() == 0 ) ;  
        REQUIRE ( node->GetNodeEndPosition() == 45 ) ; 

    }

    SECTION( "lambda a = 1, b, c, /, d, **h, : x " )
    {

        auto sourceBuffer = std::make_shared<SourceBuffer>( std::make_shared<std::wstring>( L"lambda a = 1, b, c, /, d, **h, : x " ) );
        auto lexer = std::make_shared<PythonCoreTokenizer>(4, sourceBuffer);
        auto parser = std::make_shared<PythonCoreParser>(lexer);
        ...
        auto left = std::static_pointer_cast<AST::VarArgsListExpressionNode>( node->GetLeft() );
        REQUIRE( left->GetNodes()->size() == 4 );
        REQUIRE( left->GetSeparators()->size() == 6 );
        REQUIRE( left->GetSeparators()->at(0)->GetSymbolKind() == TokenKind::PyComma );
        REQUIRE( left->GetSeparators()->at(1)->GetSymbolKind() == TokenKind::PyComma );
        REQUIRE( left->GetSeparators()->at(2)->GetSymbolKind() == TokenKind::PyComma );
        REQUIRE( left->GetSeparators()->at(3)->GetSymbolKind() == TokenKind::PyComma );
    ...
    SECTION( "lambda *a, b : x " )
    {

        auto sourceBuffer = std::make_shared<SourceBuffer>( std::make_shared<std::wstring>( L"lambda *a, b : x " ) );
        auto lexer = std::make_shared<PythonCoreTokenizer>(4, sourceBuffer);
        auto parser = std::make_shared<PythonCoreParser>(lexer);
        ...
        auto el1 = std::static_pointer_cast<AST::AtomNameNode>( left->GetNodes()->at(0) );
        REQUIRE( el1->GetNameText()->GetText()->compare(L"b") == 0 );
    ...
    SECTION( "lambda *a, b, **c, : x " )
    {
        ...
        REQUIRE( left->GetPower()->GetSymbolKind() == TokenKind::PyPower );
        auto el9txt = std::static_pointer_cast<NameToken>( left->GetPowerNode() );
        REQUIRE( el9txt->GetText()->compare(L"c") == 0 );
